...
class InvestInfo:
    ...
    class UserTransaction:
        ...
        def print(self):
            print("Date: {}, Shares: {}, Cash Paid: {}, Name: {}".format(self.date, self.shares_bought, self.cash_paid, self.investor_name))
    ...
    def add_investment(self, date, shares, cash, i_name):
        ...
        if self.user_records.get(i_name) == None:
            ...
            for index in range(len(self.user_records[i_name])):
                if len(self.user_records[i_name]) == 1:
                    if self.user_records[i_name][0].date > new_transact.date:
                        self.user_records[i_name].insert(0, new_transact)
                        added_item = True
                        break
                if self.user_records[i_name][index].date > new_transact.date and index > 0:
                    self.user_records[i_name].insert(index - 1, new_transact)
                    added_item = True
            if added_item == False:
    ...
    def get_investments_by_date(self, date):
        ...
        for user in user_date_info:
            user_date_info[user].append(round(user_date_info[user][0]/total_shares * 100, 2))
            ownership_list.append({"investor": user, "shares": user_date_info[user][0], "cash_paid": user_date_info[user][1], "ownership": user_date_info[user][2]})
...
if __name__ == "__main__":
    if len(sys.argv) == 2 or len(sys.argv) == 3:
        ...
        print("usage: csv_to_aggregate_json.py input_file [date]")